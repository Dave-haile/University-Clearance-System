...
import { FromFields,schema } from "../components/Login/Schema";
...
const Login: React.FC = () => {
  ...
  return (
    <div className={`dark:bg-[#1F2937] bg-[#F9FAFB] overflow-hidden h-screen`}>
      <div className="flex items-center justify-center gap-x-[9rem] text-[#2563eb] dark:text-[dodgerblue] font-semibold mt-5 p-1">
        <Link to={"/"}>
          <div className="cursor-pointer flex items-center text-xs hover:underline">
            <MdArrowBackIos />
            Back to Home
          </div>
        </Link>
      ...
      >
        ...
        <button
          ...
          type="submit"
          className="w-[270px] h-[30] xs:w-[360px] xs:h-[40px] md:w-[450px] md:h-[50px] p-2 md:p-0 hover:bg-[#235ede] bg-[#2563eb] text-white text-base font-medium md:font-semibold rounded-full mt-5 md:mt-4"
        >
          {isSubmitting ? "Loading..." : "Submit"}
        </button>
      </form>
      <div className="flex items-center justify-center mt-5 text-gray-500 dark:text-gray-50">
        <div className="border-[1px] w-[200px] border-gray-300 mr-1" />
        <span className="dark:text-gray-50 text-gray-500"> OR</span>
        <div className="border-[1px] w-[200px] border-gray-300 ml-1"></div>
      </div>
      <div className="flex flex-col items-center">
        <button className="w-[270px] h-[30] sm:w-[360px] sm:h-[40px] md:w-[450px] md:h-[50px] p-2 md:p-0 bg-[#e4e4e4] text-black text-base font-medium rounded-full mt-5 md:mt-4 flex items-center justify-center">
          <img
        ...
        <p className="text-sm text-gray-700 dark:text-gray-400 mb-4 mt-8">
          Don’t have an account? Ask Admin or Department Head for your details
        </p>
        <Link
          className="mt-4 text-[#2563eb] dark:text-[dodgerblue] hover:underline"
          to="/forget-password"
        >
          Forgot password
        </Link>
