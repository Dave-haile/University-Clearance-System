...
import { z } from 'zod';
import { Loader2, Check, User, Fingerprint, Calendar, Copy, CheckCircle } from 'lucide-react';
import CollegeDepartmentSelect from './CollegeDepartmentSelect';
...
const StudentRegistrationForm: React.FC<StudentRegistrationFormProps> = ({ onCancel, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [returnData, setReturnData] = useState<{ message: string; username: string; password: string } | null>(null);
  ...
  const onSubmit = async (data: FormFields) => {
    ...
    setTimeout(() => {
      setReturnData({
        message: "Academic student profile has been provisioned.",
        username: data.student_id.toUpperCase().replace(/[^A-Z0-9]/g, ''),
  ...
  const handleCopy = () => {
    if (returnData) {
      navigator.clipboard.writeText(`Academic Account\nUsername: ${returnData.username}\nPassword: ${returnData.password}`);
      setCopied(true);
    ...
    return (
      <div className="py-6 flex flex-col items-center text-center gap-6 animate-fade-in">
        ...
        <div className="space-y-1">
          <h4 className="text-xl font-bold text-slate-900 dark:text-slate-50">Student Profile Created</h4>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-[300px] mx-auto">The registration process is complete. Securely relay these credentials to the user.</p>
        </div>
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 relative overflow-hidden group">
          ...
          <div className="flex flex-col gap-5 text-left">
            <div className="space-y-1">
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Access ID</span>
              <p className="text-sm font-mono font-bold text-slate-900 dark:text-slate-100">{returnData.username}</p>
            </div>
            <div className="h-px bg-slate-200 dark:bg-slate-700 w-full" />
            <div className="space-y-1">
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Generated Secret</span>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-5">
        ...
        <div className="bg-indigo-50/50 dark:bg-indigo-950/20 p-4 rounded-2xl border border-indigo-100/50 dark:border-indigo-900/30 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-indigo-900 dark:text-indigo-300">New Academic User</p>
            <p className="text-[10px] text-indigo-500 font-medium">Provisioning student clearing privileges</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Student Full Name</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Registration ID</label>
              <div className="relative">
              ...
              {errors.student_id && <p className="text-[10px] font-bold text-rose-500 ml-2 mt-1">{errors.student_id.message}</p>}
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Level of Study</label>
              <div className="relative">
