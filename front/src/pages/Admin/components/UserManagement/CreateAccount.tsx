...
import { z } from 'zod';
import { Loader2, Check, User, Mail, Lock, ShieldCheck, CheckCircle, Copy, Shield } from 'lucide-react';
import CollegeDepartmentSelect from "../DepartmentManagement/DepartmentCollegeFetch";
...
const StaffRegistrationForm: React.FC<StaffRegistrationFormProps> = ({ onCancel, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [returnData, setReturnData] = useState<{ message: string; email: string; password: string } | null>(null);
  ...
  const onSubmit = async (data: FormFields) => {
    ...
    setTimeout(() => {
      setReturnData({
        message: "Professional staff account successfully provisioned.",
        email: data.email,
  ...
  const handleCopy = () => {
    if (returnData) {
      navigator.clipboard.writeText(`Staff Account\nEmail: ${returnData.email}\nPassword: ${returnData.password}`);
      setCopied(true);
    ...
    return (
      <div className="py-6 flex flex-col items-center text-center gap-6 animate-fade-in">
        ...
        <div className="space-y-1">
          <h4 className="text-xl font-bold text-slate-900 dark:text-slate-50">Staff Account Created</h4>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-[300px] mx-auto">Access has been granted. Share these credentials with the staff member.</p>
        </div>
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 relative overflow-hidden group">
          ...
          <div className="flex flex-col gap-5 text-left">
            <div className="space-y-1">
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Work Email</span>
              <p className="text-sm font-mono font-bold text-slate-900 dark:text-slate-100">{returnData.email}</p>
            </div>
            <div className="h-px bg-slate-200 dark:bg-slate-700 w-full" />
            <div className="space-y-1">
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Access Password</span>
              <p className="text-sm font-mono font-bold text-slate-900 dark:text-slate-100">{returnData.password}</p>
            </div>
          </div>
        </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-5">
        ...
        <div className="bg-violet-50/50 dark:bg-violet-950/20 p-4 rounded-2xl border border-violet-100/50 dark:border-violet-900/30 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-violet-900 dark:text-violet-300">Staff Access Provisioning</p>
            <p className="text-[10px] text-violet-500 font-medium">Configuring operational permissions</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Full Professional Name</label>
            ...
            {errors.name && <p className="text-[10px] font-bold text-rose-500 ml-2 mt-1">{errors.name.message}</p>}
          </div>

          {/* Email */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Institutional Email</label>
            <div className="relative">
            ...
            {errors.email && <p className="text-[10px] font-bold text-rose-500 ml-2 mt-1">{errors.email.message}</p>}
          </div>

          {/* Role */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Assigned Operational Role</label>
            <div className="relative">
            ...
            {errors.role && <p className="text-[10px] font-bold text-rose-500 ml-2 mt-1">{errors.role.message}</p>}
          </div>
          ...
          {/* Password */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Access Password</label>
            <div className="relative">
            ...
            {errors.password && <p className="text-[10px] font-bold text-rose-500 ml-2 mt-1">{errors.password.message}</p>}
          </div>
        </div>
      </div>
